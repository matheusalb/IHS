...
namespace HexaPawn {
	...
	int get_choice(int limit = 4){
		int choice = de150.readButton();
		while(choice < 0 || limit <= choice)
			choice = de150.readButton();
		printf("escolheu %d\n", choice);
		return choice;
	}
	...
	bool cpu_play(bool show, string state = "222000111", int turn = 0){
		...
		if(turn == 0){
			...
			for(int i=0; i<possibleMoves.size(); i++){
				if(cpu_play(show, possibleMoves[i], 1)) any_victory = 1;
			}
			return any_victory;
		}
		else{
